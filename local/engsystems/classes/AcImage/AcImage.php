...
class AcImage
{
 ...
 /**
    ...
    /**
     * Вписывает изображение в рамки и обрезает лишнее.
     * Если картинка меньше рамки, то увеличивает её.
     * Принимает размер (рамку) в которую вписывается изображение
     * высоту и ширину этой рамки.
     *
     * @return AcImage
     * @throws IllegalArgumentException
     */

    public function scaleAndCrop2($width, $height)
    {
        if (($this->getWidth() / $this->getHeight()) > ($width / $height)) {
            $newWidth = (int) ceil($this->getWidth() * $height / $this->getHeight());
            $newHeight = $height;
        } else {
            $newWidth = $width;
            $newHeight = (int) ceil($this->getHeight() * $width / $this->getWidth());
        }

        $newImageResource = imagescale($this->getResource(), $newWidth, $newHeight, IMG_BICUBIC);
    ...
    public function scaleAndCrop($newWidth, $newHeight)
    {
    	$sourceWidth = $this->getWidth();
		$sourceHeight = $this->getHeight();

		if (($sourceWidth / $sourceHeight) > ($newWidth / $newHeight)) {
			$resizeHeight = (int) $newHeight ;
			$resizeWidth = (int) ($newHeight / $sourceHeight * $sourceWidth);
		} else {
			$resizeWidth = (int) $newWidth;
			$resizeHeight = (int) ($newWidth / $sourceWidth * $sourceHeight);
		}

		$image_resize_blank = imagecreatetruecolor($resizeWidth, $resizeHeight);
		imageAlphaBlending($image_resize_blank, false);
		imageSaveAlpha($image_resize_blank, true);
		$image_resize_source = $this->resource;
		$image_resize_resampled = imagecopyresampled($image_resize_blank, $image_resize_source, 0, 0, 0, 0, $resizeWidth, $resizeHeight, $sourceWidth, $sourceHeight);

		if ( !$image_resize_resampled ) {
			throw new IllegalArgumentException();
		}

		$delta_x = $resizeWidth - $newWidth;
		$delta_y = $resizeHeight - $newHeight;
		...
		$image_crop_blank = imagecreatetruecolor($newWidth, $newHeight);
		imageAlphaBlending($image_crop_blank, false);
		imageSaveAlpha($image_crop_blank, true);
		$image_crop_resampled = imagecopyresampled($image_crop_blank, $image_resize_blank, 0, 0, $src_x, $src_y, $newWidth, $newHeight, $newWidth, $newHeight);

		if ( !$image_crop_resampled ) {
			throw new IllegalArgumentException();
		}

		$this->setResource($image_crop_blank);
		$this->setSize(new Size($newWidth, $newHeight));
		return $this;
    }
