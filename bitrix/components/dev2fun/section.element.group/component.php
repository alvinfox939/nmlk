...
if($this->StartResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter, $arSectionNavParams, $arrSectionFilter, $arParams)))
{
	...
	if($arResult = $rsIBlock->GetNext())
	{
		...
		if(!array_key_exists("ID", $arSortSection))
            $arSortSection["ID"] = "DESC";
		...
		if(!empty($arParams["SECTION_ID"])){
			$arrSectionFilter["ID"] = $arParams["SECTION_ID"];
		}

		if(!empty($arParams["SECTION_CODE"])){
			$arrSectionFilter["CODE"] = $arParams["SECTION_CODE"];
		}
...
//				array("SECTION_BUTTONS"=>false, "SESSID"=>false)
        ...
        $arResult["ITEMS"] = $this->getElementsSection(
            array(
                'arSort' => $arSort,
                'arFilter' => array_merge($arFilter, $arrFilter),
                'arNavParams' => $arNavParams,
