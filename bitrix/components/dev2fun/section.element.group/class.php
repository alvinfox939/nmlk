...
//			$arFilter["INCLUDE_SUBSECTIONS"] = "Y";
	...
	*/
	public final function conditionResultArray($arrItems, $pathGroupKeys='SECTION_RESULT/ID'){
		...
		foreach($arrItems as $key=>$item){
			...
			$arrRes = null;
			if(!empty($result[$thisKey][0])) {
                $arrRes = $result[$thisKey][0];
            }
			$ifs = $this->recResultArray($item,$groupPath,$arrRes);
	...
	* @return true|false
	*/
	private function recResultArray($item,$groupPath,$arrRes,$step=0){
	...
	*/
	 ...
	 */
	public function recRender($params,$type='TEMP_OUTPUT_SECTIONS', $keyName='arSectionsChild')
	{
		...
		$this->ob(
			$params['path'].$params['arParams'][$type],
			$params['arResult'],
            $keyName
		);
	}
